...
# Temp file is used as a super global variable
# to permit for access by other scripts and to
# solve scope issues
_IS_HIDDEN_FILE=""
function is_hidden() {
	if [ $# -eq 0 ]; then
...
usage() {
    ...
    printf "   --region and --hover will be ignored.\n"
    printf "\n"
    ...
    printf "\n"
    printf "--show\n"
    printf "Like --toggle, but forces the window to show.\n"
    printf "\n"
    printf "--hide\n"
    printf "Like --toggle, but forces the window to hide.\n"
    printf "\n"
    printf " -t, --toggle\n"
